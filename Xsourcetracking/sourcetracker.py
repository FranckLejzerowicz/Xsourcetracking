...
def run_sourcetracker(
    ...
    for t in range(p_times):
        ...
        if loo:
            ...
            meta_list = []
            for sam in samples[sink]:
                meta_list.append([sam, 'source', sink])
            for source in sources:
                for sam in samples[source]:
                    meta_list.append([sam, 'source', source])
            meta = pd.DataFrame(meta_list, columns=['#SampleID', 'SourceSink', 'Env'])
            map_out = '%s/map.loo.tsv' % o_dir_path_meth_t
            meta.to_csv(map_out, index=False, sep='\t')

            cur_p_cpus = p_cpus
            if p_cpus > len(samples[sink]):
                cur_p_cpus = len(samples[sink])

            o_dir_path_meth_t_r = o_dir_path_meth_t + '/loo'
